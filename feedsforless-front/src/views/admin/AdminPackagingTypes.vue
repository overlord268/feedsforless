<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Packaging types</h1>
      ...
      <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
          <div>
            ...
            <input v-model="form.name" type="text" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900" placeholder="e.g. Bag" />
          </div>
...
async function fetchItems() {
  loading.value = true;
  try {
    const { data } = await api.get('/api/v1/admin/packaging-types');
    const raw = data?.data ?? data;
    items.value = Array.isArray(raw) ? raw : (raw?.data ?? []);
  } catch (e) { console.error(e); items.value = []; } finally { loading.value = false; }
}

function openAdd() { editingId.value = null; form.name = ''; showModal.value = true; }
function openEdit(item) { editingId.value = item.id; form.name = item.name ?? ''; showModal.value = true; }

async function submitForm() {
  saving.value = true;
  try {
    if (editingId.value) {
      await api.put(`/api/v1/admin/packaging-types/${editingId.value}`, { name: form.name });
      setSuccess('Packaging type updated.');
    } else {
      await api.post('/api/v1/admin/packaging-types', { name: form.name });
      setSuccess('Packaging type created.');
    }
...
async function confirmDelete(item) {
  const ok = await useConfirm().show({ title: 'Delete packaging type', message: `Delete "${item.name}"?`, confirmLabel: 'Delete', cancelLabel: 'Cancel', variant: 'danger' });
  if (!ok) return;
  try {
    await api.delete(`/api/v1/admin/packaging-types/${item.id}`);
    setSuccess('Packaging type deleted.');
    await fetchItems();
