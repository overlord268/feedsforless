<template>
  <div class="space-y-5">
    ...
    <CrudTable
      ...
      empty-message="No categories."
    >
      <template #row="{ item: cat }">
        <td class="px-4 py-2.5 text-slate-700">{{ cat.id }}</td>
        <td class="px-4 py-2.5 text-slate-800">{{ cat.label }}</td>
        <td class="px-4 py-2.5 text-slate-600 font-mono text-xs">{{ cat.slug }}</td>
        <td class="px-4 py-2.5 text-slate-600">{{ cat.parent_id ? `#${cat.parent_id}` : '—' }}</td>
...
const columns = [
  ...
  { key: 'slug', label: 'Slug' },
  { key: 'parent_id', label: 'Parent', sortValue: (c) => c.parent_id ?? 0 },
