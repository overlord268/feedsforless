<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Measure units</h1>
        <p class="text-slate-500 mt-0.5">Technical config.</p>
      </div>
      <button type="button" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 text-white font-medium hover:bg-emerald-700 transition" @click="openAdd">+ Add unit</button>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" @click.self="showModal = false">
      <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        <h2 class="text-lg font-semibold text-slate-900 mb-4">{{ editingId ? 'Edit measure unit' : 'Add measure unit' }}</h2>
        <form @submit.prevent="submitForm" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Label</label>
            <input v-model="form.label" type="text" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900" placeholder="e.g. Kilogram" />
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Notation</label>
            <input v-model="form.notation" type="text" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900" placeholder="e.g. kg" />
          </div>
...
function openAdd() { editingId.value = null; form.label = ''; form.notation = ''; showModal.value = true; }
function openEdit(item) { editingId.value = item.id; form.label = item.label ?? ''; form.notation = item.notation ?? ''; showModal.value = true; }

async function submitForm() {
  saving.value = true;
  try {
    if (editingId.value) {
      await api.put(`/api/v1/admin/measure-units/${editingId.value}`, { label: form.label, notation: form.notation || null });
      setSuccess('Measure unit updated.');
    } else {
      await api.post('/api/v1/admin/measure-units', { label: form.label, notation: form.notation || null });
      setSuccess('Measure unit created.');
    }
...
async function confirmDelete(item) {
  const ok = await useConfirm().show({ title: 'Delete measure unit', message: `Delete "${item.label}"?`, confirmLabel: 'Delete', cancelLabel: 'Cancel', variant: 'danger' });
  if (!ok) return;
  try {
    await api.delete(`/api/v1/admin/measure-units/${item.id}`);
    setSuccess('Measure unit deleted.');
    await fetchItems();
