...
const columns = MEASURE_UNITS_COLUMNS;
const modalTitle = computed(() =>
  editingId.value ? 'Edit measure unit' : 'Add measure unit'
);
...
async function submitForm() {
  ...
  try {
    if (editingId.value) {
      await api.put(`/api/v1/admin/measure-units/${editingId.value}`, {
        label: form.label,
        notation: form.notation || null,
      });
      setSuccess('Measure unit updated.');
    } else {
      await api.post('/api/v1/admin/measure-units', {
        label: form.label,
        notation: form.notation || null,
      });
