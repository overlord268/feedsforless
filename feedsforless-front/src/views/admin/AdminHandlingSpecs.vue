<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Handling specs</h1>
        <p class="text-slate-500 mt-0.5">Technical config.</p>
      </div>
      <button type="button" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 text-white font-medium hover:bg-emerald-700 transition" @click="openAdd">+ Add spec</button>
    </div>
        ...
        <table class="w-full text-sm min-w-[600px]">
          <thead class="bg-slate-50/80 border-b border-slate-200">
            <tr class="text-left">
              ...
              <th class="px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Requirement</th>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" @click.self="showModal = false">
      <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        <h2 class="text-lg font-semibold text-slate-900 mb-4">{{ editingId ? 'Edit handling spec' : 'Add handling spec' }}</h2>
        <form @submit.prevent="submitForm" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Label</label>
            <input v-model="form.label" type="text" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900" placeholder="e.g. Refrigerated" />
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Requirement</label>
            <input v-model="form.requirement" type="text" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900" placeholder="Optional" />
          </div>
...
async function fetchItems() {
  loading.value = true;
  try {
    const { data } = await api.get('/api/v1/admin/handling-specs');
    const raw = data?.data ?? data;
    items.value = Array.isArray(raw) ? raw : (raw?.data ?? []);
  } catch (e) { console.error(e); items.value = []; } finally { loading.value = false; }
}

function openAdd() { editingId.value = null; form.label = ''; form.requirement = ''; showModal.value = true; }
function openEdit(item) { editingId.value = item.id; form.label = item.label ?? ''; form.requirement = item.requirement ?? ''; showModal.value = true; }
...
async function confirmDelete(item) {
  const ok = await useConfirm().show({ title: 'Delete handling spec', message: `Delete "${item.label}"?`, confirmLabel: 'Delete', cancelLabel: 'Cancel', variant: 'danger' });
  if (!ok) return;
  try {
    await api.delete(`/api/v1/admin/handling-specs/${item.id}`);
    setSuccess('Handling spec deleted.');
    await fetchItems();
